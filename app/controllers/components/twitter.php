		 ...
		 */		
		 public function setup($consumer_key, $consumer_secret, $cookie) {
			...
			if($cookie == true) {
				...
				$oauth_cookie = $_COOKIE['Twitter.OAuth.Consumer'];
			...
			$_SESSION['Twitter.OAuth.Consumer']= $cookie_content;
		 }
		 ...
		 public function setOauthConsumerKeys($consumer_key, $consumer_secret, $cookie) {
			...
			if($cookie == true) {
				if(!is_null($_COOKIE('Twitter.OAuth.Consumer'))) $this->Cookie->delete('Twitter.OAuth.Consumer');
