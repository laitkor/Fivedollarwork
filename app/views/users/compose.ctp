<?php $this->set('title_for_layout', 'Compose Message');?><h3 class="blog-title"><a href="#" title="Profile Settings">Compose Message</a></h3>
					...
					<form class="wpsc_checkout_forms" method="post" name="Compose" action="/users/compose/<?php echo $user['User']['id']; ?>/<?php echo $gig['Recommend']['id']; ?>" enctype="multipart/form-data" id="Compose" >
			  ...
			  <tr><td><b>From : </b></td>
			  <td><?php if(is_numeric($user['User']['username'])) {echo $user['User']['name']; }else {echo$user['User']['username']; }  ?></td>
              </tr>
			<tr><td><b> To : </b></td><td><?php echo $gig['User']['username']; ?></td></tr>
            <tr><td><b>Recommend : </b></td><td><?php echo $gig['Recommend']['recommend'] ?></td></tr>
