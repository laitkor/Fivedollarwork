<?php /*$this->set('title_for_layout', '');*/?><!--<h3 class="blog-title"><a href="#" title="Profile Settings">Gigs</a></h3>
				 ...
				 <table cellspacing="0" cellpadding="0"  class="payment" width="100%">
                        <tbody>
							...
							<td class="row"  style="background-color: #A6BA76;  color: #000; " ><a href="/gigs/bestrating" style="color:#000"><b>Best Ratings</b></a></td>
	...
	if(count($allgigs)>0)
		{ //echo "<pre>";print_r($allUser);
			...
			<td>
            <table width="100%"><tr><td colspan="4"><a href="ordergig/<?php echo $gigs['Gig']['id'] ?>"><h4 class="blog-title">I Will <?php echo $gigs['Gig']['title'];?> for $<?php echo $gigs['Gig']['price'];?></h4></a></td></tr>
            <tr><td colspan="5"><?php echo substr($gigs['Gig']['description'],0,50);?>.......<br />(by <?php  if(is_numeric($gigs['User']['username'])) {echo $gigs['User']['name']; }else {echo $gigs['User']['username']; } ?>)</td></tr>
...
<td><a href="#">Share</a></td><td><a href="http://www.facebook.com/share.php?u=<?php echo SITE_URL; ?>/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>" onclick="return fbs_click()" target="_blank"><img src="/images/icon-facebook.png" alt="Share on Facebook" /></a>
              ...
              <td><input type="button" value="Order" class="broap"  id="<?php echo $gigs['Gig']['id']; ?>" onclick="return order(this.id);"  /></td></tr>
...
?>
			<tr><td colspan="2"><?php echo $this->element('pagination'); //call pagination from element ?></td></tr>		
