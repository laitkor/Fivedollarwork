<?php /* $this->set('title_for_layout', '5$ Work');*/?><!--<h3 class="blog-title"><a href="#" title="Profile Settings">5$ Work</a></h3>
				 ...
				 <table cellspacing="0" cellpadding="0"  class="payment" width="100%">
                	...
                	<option  value="<?php echo $price ?>" selected="selected"><?php echo '$'.$price ?></option>
                    <?php if(isset($gigsprice)){foreach($gigsprice as $prices){ ?>
                    <?php if($prices['Gigsprice']['price']!=$price){ ?>
                    <option value="<?php echo $prices['Gigsprice']['price']; ?>">$<?php echo $prices['Gigsprice']['price']; ?></option>
                    <?php }} }?>
                </select>
</td>
    	  ...
    	  { 
			echo $html->link($html->image('/images/unlike.png'),array('action' => 'like',
			 $gigs['Gig']['id']),array( 'update' => 'post','escape'=>false,'title'=>'Unlike')); 
              ...
              <td align="right"><input type="button"  value="Order" class="broap"  id="<?php echo $gigs['Gig']['id']; ?>" onclick="return order(this.id);"  /></td></tr>
