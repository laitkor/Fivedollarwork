<?php /*$this->set('title_for_layout', '5$ Work');*/?><!--<h3 class="blog-title"><a href="#" title="Profile Settings">5$ Work</a></h3>-->
				...
				<form class="wpsc_checkout_forms" method="post" name='Ordergig' id="Ordergig" >
				 <table cellspacing="0" cellpadding="0"  class="payment" width="100%">
                        <tbody>
                            ...
                            <td class="row" style="width:25%"  align="right">Job Price:&nbsp;&nbsp;<select name="data[price]" required="required" onchange="getPrice(this.value)">
	...
	if(count($allgigs)>0)
		{ 
			...
			?>
			  <tr class="rowgrid">
			  <td class="frame"><a href="/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>"><img src="/uploads/gig_img/<?php if($gigs['Gig']['image']==''){?>noimage.jpg <?php } else { echo $gigs['Gig']['image']; } ?>" title="Gigs" width="120px"  height="80px" />
               <?php if($gigs['Gig']['featured']=='Y'){ ?> <a href="/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>"><span class="featured-label">featured</span></a> <?php } ?>
              </a></td>
			<td>
            <table width="100%">
            <tr><td colspan="4">
            <a href="/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>"><h4 class="blog-title">
            I Will <?php echo $gigs['Gig']['title'];?> for $<?php echo $gigs['Gig']['price'];?>
            </h4></a></td></tr>
            <tr><td colspan="4"><?php echo substr($gigs['Gig']['description'],0,50);?>&nbsp;.......<br />(by <?php 
			if(is_numeric($gigs['User']['username'])) {echo $gigs['User']['name']; }else {echo $gigs['User']['username'];}
			 ?>)</td></tr><tr><td>&nbsp;</td></tr>
            <tr><td id="post"><a href="/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>"><b>Read More</b></a>
            &nbsp;<?php
    	  ...
    	  { 
			echo $html->link($html->image('/images/unlike.png'),array('action' => 'like',
			 $gigs['Gig']['id']),array( 'update' => 'post','escape'=>false,'title'=>'Unlike')); 
			  ...
			  ?><td><b>Share</b></td><td>
            			  <a href="http://www.facebook.com/share.php?u=<?php echo SITE_URL; ?>/gigs/ordergig/<?php echo $gigs['Gig']['id'] ?>" onclick="return fbs_click()" target="_blank"><img src="/images/icon-facebook.png" alt="Share on Facebook" /></a>
